...
# ======================================================================================
# finetune_coptidice.sh
#
# Description:
#   This script launches parallel fine-tuning jobs for the coptidice algorithm.
#   It finds pretrained models based on a task name and a list of seeds,
#   and then runs train_coptidice.py for a new range of seeds.
#
# Usage:
#   ./finetune_coptidice.sh --task <task_name> [options]
#
# Example:
#   ./finetune_coptidice.sh \
...
if [ -z "$TASK" ]; then
    ...
    echo "Usage: ./finetune_coptidice.sh --task <task_name> [--pretrained_seeds \"1 5 10\"] [--new_seeds \"21 22 23\"] [--max_jobs 4]"
    exit 1
fi
...
for pretrained_seed in $PRETRAINED_SEEDS; do
    ...
    search_pattern="${BASE_LOG_DIR}/${TASK}/COptiDICE_cost${COST_LIMIT}_seed${pretrained_seed}*/COptiDICE_cost${COST_LIMIT}_seed${pretrained_seed}*"
    ...
    for pretrained_path in "${pretrained_paths[@]}"; do
        ...
        for new_seed in $NEW_SEEDS; do
            ...
            (
              python -m train.train_coptidice \
                --device "cuda" \
