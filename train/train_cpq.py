...
from offpolicy.cpq import CPQFinetunePolicy
from config.cpq_configs import CPQFinetuneConfig


@pyrallis.wrap()
def finetune(args: CPQFinetuneConfig):
    ...
    cpq_model = CPQ(
        ...
        qc_scalar=original_cfg["qc_scalar"],
    ...
    cpq_model.load_state_dict(model_state["model_state"])
    cpq_model.to(args.device)

    # IMPORTANT: Initialize new optimizers for fine-tuning
    cpq_model.setup_optimizers(
        args.actor_lr, args.critic_lr, args.alpha_lr, args.vae_lr
    )

    # 5. Initialize Policy
    policy = CPQFinetunePolicy(
        model=cpq_model,
